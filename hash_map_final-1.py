...
class HashMap:
    ...
    def resize_table(self, capacity):
        ...
        for bucket in self._buckets:
            current = bucket.head  # start with the head of the linked list
            while current is not None:  # traverse the linked list
                temp.put(current.key, current.value)  # rehash and add to the new table
                current = current.next  # move to the next node

        # point the existing hashMap to the temporary hashMap and change capacity as well
        self._buckets = temp._buckets  # replace the buckets with the resized buckets
        self.capacity = temp.capacity  # update capacity
        self.size = temp.size  # update size to match the temporary hash map
